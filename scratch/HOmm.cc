...
void reportParams( NetDeviceContainer ueDevs, uint16_t indx , Ptr<sdnClient> ch,ns3::Ptr<ns3::RandomWalk2dMobilityModel> randomWalkModel){
  ...
  std::string ueSpeed = std::__cxx11::to_string(vv.x);
  ueSpeed += "|";
  // std::cout<<"Speed received is "<<ueSpeed<<"\n";
  // std::cout<<"Packet in string format is "<<params.length()<<"\n";
  char data[ params.length() ];
  char speedString[ueSpeed.length()];
  std::strcpy(data,params.c_str());
  std::strcpy(speedString, ueSpeed.c_str());
  ch->SendData(data , params.length()+1);
  ch->SendData(speedString,ueSpeed.length()+1);
  // std:: cout  << "Reporting.. || UE :: " << 0 << " | CELL :: " << indx << " | SINR :: " << ueDevs.Get(0)->GetObject<MmWaveUeNetDevice> ()->GetPhy()->frqAvg[indx].second / 15 << "\n";
  Simulator::Schedule ( MilliSeconds(200) , &reportParams , ueDevs , indx, ch, randomWalkModel );
}
...
main (int argc, char *argv[])
{
 ...
 Ptr<MmWaveHelper> mmwaveHelper = CreateObject<MmWaveHelper> ();
  ...
  for(int i=0;i<numberOfUes;i++){
    Simulator::Schedule (Seconds(0.1) , &reportParams , ueDevs , i, sdnClients[i], randomWalkModel );
  }
