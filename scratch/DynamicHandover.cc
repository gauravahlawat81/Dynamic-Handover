...
void checkProg(Ptr<NetDevice> targetEnbDev , Ptr<NetDevice> ueDev, Ptr<NetDevice> sourceEnbDev){
  std::cout << "About ------------------" << "\n";
...
void incrementRsrq(uint16_t val)
{
  std::cout<<"At time "<<Simulator::Now().GetMilliSeconds()<<" previous value "<<val<<" and new value";
  val+=2;
  std::cout<<val<<"\n";
}
void
...
main (int argc, char *argv[])
{
  ...
  double simTime = 5; // 1500 m / 20 m/s = 75 secs
...
// sourceRrc->SendHandoverRequest (rnti, targetCellId);
  ...
  // Simulator::Schedule (MilliSeconds(200), &checkProg, enbDevs.Get(1) , ueDevs.Get(0) , enbDevs.Get(0) );
  uint16_t timeInMs = simTime * 1000;
  for(int i=0;i<timeInMs;i+=20)
  {
    rsrq_val+=2;
    if(rsrq_val>200)
    {
      std::cout<<"Triggering handover because value reached a offset";
      Simulator::Schedule(MilliSeconds(i),&checkProg,enbDevs.Get(1),ueDevs.Get(0),enbDevs.Get(0));
      break;
    }
  }

  // Simulator::Schedule (MilliSeconds(200), &incrementRsrq,rsrq_val);
  ...
  AnimationInterface anim("DynamicHO.xml");
