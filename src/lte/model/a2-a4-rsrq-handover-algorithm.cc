          ...
          if ((it2->second->m_rsrq > bestNeighbourRsrq)
              && IsValidNeighbour (it2->first))
            {
              ...
              bestNeighbourRsrq = it2->second->m_rsrq;
                      ...
                      std::cout<<"A2A4 RSRP Values "<<(uint16_t)it2->second->m_rsrp<<"\n";
                      std::cout<<"A2A4 RSRQ Values "<<(uint16_t)it2->second->m_rsrq<<"\n";
            }
        }
          ...
          if ((bestNeighbourRsrq - servingCellRsrq) >= m_neighbourCellOffset)
            {
              ...
              std::cout<<"Trigger Handover to cellId " << bestNeighbourCellId<<"\n";
              std::cout<<"target cell RSRQ " << (uint16_t) bestNeighbourRsrq<<"\n";
              std::cout<<"serving cell RSRQ " << (uint16_t) servingCellRsrq<<"\n";
